...
exports.run = async (client, message, args) => {
    ...
    if (data.serverID === message.guild.id) {
        ...
        if (channelID) {
            ...
            embed.setDescription('Update Channel Setup')
            embed.addField('Update Channel has been set to:', `${channelID}`)
            return message.channel.send({ embed });
        } else {
            const alertEmbed = require('../embeds/alertEmbed');
            const embed = new Discord.MessageEmbed(alertEmbed);

            embed.setDescription('Incorrect usage of setUpdateChannel');
            embed.addField('Use like this:', '!setUpdateChannel <Channel ID here>');
            return message.channel.send({ embed });
        }
    } else {
...
exports.help = {
    ...
    description: `Set the Update Channel for your server`,
    usage: 'setUpdateChannel <Channel ID here>'
};

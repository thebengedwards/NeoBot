...
exports.run = async (client, interaction, options, perms) => {
  if (!options) {
    ...
    embed.addFields(
      ...
      { name: `Games`, value: client.commands.filter(item => item.conf.permLevel <= perms).map(item => item.help.name) },
    ...
    if (client.commands.has(command)) {
      ...
      embed.setDescription(`Help with: ${clientCommand.help.name}`)
      embed.addFields(
        { name: 'Name', value: `${clientCommand.help.name}` },
        { name: 'About', value: `${clientCommand.help.description}` },
        { name: 'Fields', value: `${clientCommand.help.options ? clientCommand.help.options.length : '0'}` },
      )
      Reply(client, interaction, embed)

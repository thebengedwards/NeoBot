...
exports.run = async(client, message, args) => {
    ...
    if (data.serverID === message.guild.id) {
        ...
        if (channelID) {
            ...
            embed.setDescription('Memes Channel Setup')
            embed.addField('Memes Channel has been set to:', `${channelID}`)
            return message.channel.send({ embed });
        } else {
            const alertEmbed = require('../embeds/alertEmbed');
            const embed = new Discord.MessageEmbed(alertEmbed);

            embed.setDescription('Incorrect usage of setMemesChannel');
            embed.addField('Use like this:', '!setMemesChannel <Channel ID here>');
            return message.channel.send({ embed });
        }
    } else {
...
exports.help = {
    ...
    description: `Set the Memes Channel for your server`,
    usage: 'setMemesChannel <Channel ID here>'
};
